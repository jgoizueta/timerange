...
function yearWeek (y, yd) {
    ...
    if (yd < start) {
        ...
        yd += Math.round((dateValue(y, 1, 1) - dateValue(y - 1, 1, 1)) / MS_PER_DAY);
        return yearWeek(y - 1, yd);
    } else if (dateValue(y, 1, 1) + (yd - 1 + 3) * MS_PER_DAY >= dateValue(y + 1, 1, 1)) {
        // The Wednesday (start of week + 3) lies in the next year => advance one year
        yd -= Math.round((dateValue(y + 1, 1, 1) - dateValue(y, 1, 1)) / MS_PER_DAY);
        return yearWeek(y + 1, yd);
    }
...
function timeStart(i, fmt) {
    return i == fmt.unit ? fmt.base : TIME_STARTS[i];
}
...
function roundDateComponents(components, resolution, rounding) {
    if (resolution == 'week') {
        ...
        let c0 = Object.keys(TIME_LEVELS).map(i => components[i] - timeStart(i, fmt));
        ...
        if (excess.some(x => x > 0)) {
            ...
            components = Object.keys(c0).map(i => c0[i] + timeStart(i, fmt));
        }
    }

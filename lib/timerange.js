...
module.exports = class TimeRange {
     ...
     *
     * @param {String} text - Abbreviated ISO-formatted string (e.g. `'2018-03'`, `'2018-2019'`)
     * @return {TimeRange}
     * @api
     */
    ...
    /**
     * Construct a TimeRange from an time range string, allowing for open-ended intervals,
     * such as `'2018..'`, `'..2018'` or `'..'`.
     *
     * @param {String} text - Abbreviated ISO-formatted string
     * @return {TimeRange}
     * @api
     */
    static fromTextOpen (text, startOfTime=TimeInstant.fromComponents(0), endOfTime=TimeInstant.nowUTC()) {
        const sep = ABBR_INTERVAL_SEP.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&');
        if (text == ABBR_INTERVAL_SEP) {
            return TimeRange.fromStartEnd(startOfTime, endOfTime);
        }
        else if (text.match(sep + '$')) {
            const start = TimeRange.fromText(text.substring(0, text.length-2));
            const resolution = start.resolution;
            const end = endOfTime.roundUp(resolution);
            return TimeRange.fromStartEnd(start.start, end, resolution);
        }
        else if (text.match('^' + sep)) {
            const end = TimeRange.fromText(text.substring(2, text.length));
            const resolution = end.resolution;
            const start = startOfTime.roundDown(resolution);
            return TimeRange.fromStartEnd(start, end.end, resolution);
        }
        else {
            return TimeRange.fromText(text);
        }
    }
     ...
     * TimeRange.fromText('2018').in('month').text // => '2018-01..2018-12'
     *
    ...
    durationIn (units) {
        ...
        if (ufmt.unit >= DAY && fmt.unit >= DAY) {
            // exact
            return durationUnits * MS_PER[fmt.unit] / MS_PER[ufmt.unit] / ufmt.numUnits;
        }
